...
def monthly_to_dataframe():
    ...
    if not df.empty:
        df["date"] = pd.to_datetime(
            df["date"]
        )  # Python date → Timestamp for comparisons
        df["proportion_deaths"] = (df["deaths"] / df["births"]).round(6)
...
def bootstrap_chart_data_json(boot_diffs, ci_lower, ci_upper):
    ...
    return json.dumps(
        {
            "labels": [
                round((edges[i] + edges[i + 1]) / 2, 5) for i in range(len(counts))
            ],
            "counts": counts.tolist(),
            "ci_lower": round(
                ci_lower, 5
            ),  # used by charts.js to colour bars outside the CI
...
def monthly_proportion_chart_png():
    ...
    if not df.empty:
        ...
        ax.plot(
            before["date"],
            before["proportion_deaths"],
            color="tab:red",
            label="Before handwashing",
        )
        ax.plot(
            after["date"],
            after["proportion_deaths"],
            color="tab:green",
