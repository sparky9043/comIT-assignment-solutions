...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        # a few cars
        Car.objects.create(
            make="Toyota", model="Camry", year=2022, price=28500, branch=b1, seller=s1
        )
        Car.objects.create(
            make="Honda", model="Civic", year=2023, price=26000, branch=b1, seller=s2
        )
        Car.objects.create(
            make="Ford",
            model="F-150",
        ...
        Car.objects.create(
            make="BMW", model="3 Series", year=2023, price=55000, branch=b3, seller=s2
        )
